...
const COINMETRICS_COMMUNITY_API_BASE = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics";
const COINGECKO_API_BASE = "https://api.coingecko.com/api/v3";
const BGEOMETRICS_MCP_URL = "https://bitcoin-data.com/mcp";
...
const COINGECKO_API_KEY =
  ...
  process.env.x_cg_demo_api_key ??
  "";
const BGAPI_TOKEN = process.env.BGAPI_TOKEN ?? "";
...
async function fetchSourceCacheValue(cacheLabel, fetcher, ttlMs = 5 * 60 * 1000) {
  const cacheKey = toCacheKey(cacheLabel);
...
function getBGeometricsHeaders() {
  if (!BGAPI_TOKEN) {
    return {};
  }

  return {
    Authorization: `Bearer ${BGAPI_TOKEN}`,
  };
}
...
async function initializeBGeometricsMcpSession() {
  const response = await fetch(BGEOMETRICS_MCP_URL, {
    method: "POST",
    headers: {
      "content-type": "application/json",
      Accept: "application/json, text/event-stream",
      "user-agent": "btc-dashboard/bgeometrics-mcp",
      ...getBGeometricsHeaders(),
    },
    body: JSON.stringify({
      jsonrpc: "2.0",
      method: "initialize",
  ...
  if (!response.ok) {
    throw new Error(`BGeometrics MCP initialize failed ${response.status}`);
  }

  const sessionId = response.headers.get("mcp-session-id");

  if (!sessionId) {
    throw new Error("BGeometrics MCP initialize returned no session id");
  }

  return sessionId;
}

function parseBGeometricsMcpEnvelope(rawText) {
  ...
  for (const line of dataLines) {
    ...
    if (payload?.error) {
      throw new Error(payload.error.message ?? "BGeometrics MCP returned an error");
    }
  ...
  throw new Error("BGeometrics MCP returned no result payload");
}

function extractBGeometricsMcpText(result) {
...
function extractMarkdownCsv(text) {
  const match = String(text).match(/```csv\n([\s\S]*?)\n```/);
  return match?.[1] ?? null;
}
...
async function callBGeometricsMcpTool(sessionId, toolName, args = {}) {
  const response = await fetch(BGEOMETRICS_MCP_URL, {
    method: "POST",
    headers: {
      "content-type": "application/json",
  ...
  if (!response.ok) {
    throw new Error(`BGeometrics MCP tool ${toolName} failed ${response.status}`);
  }

  const rawText = await response.text();
  const result = parseBGeometricsMcpEnvelope(rawText);

  if (result?.isError) {
    throw new Error(extractBGeometricsMcpText(result) || `BGeometrics MCP tool ${toolName} returned an error`);
  }

  return extractBGeometricsMcpText(result);
}

export async function searchBGeometricsMetrics(query, options = {}) {
  return fetchSourceCacheValue(
    `bgeometrics:mcp:search:${query}`,
    async () => {
      const sessionId = await initializeBGeometricsMcpSession();
      return callBGeometricsMcpTool(sessionId, "search_metrics", { query });
    },
    options.ttlMs ?? 24 * 60 * 60 * 1000,
  );
}

export async function listBGeometricsMetrics(category, options = {}) {
  return fetchSourceCacheValue(
    `bgeometrics:mcp:list:${category ?? "all"}`,
    async () => {
      const sessionId = await initializeBGeometricsMcpSession();
      return callBGeometricsMcpTool(sessionId, "list_metrics", category ? { category } : {});
    },
    options.ttlMs ?? 24 * 60 * 60 * 1000,
  );
}

export async function fetchBGeometricsLatestMetrics(metricIds, options = {}) {
  const normalizedMetricIds = Array.from(new Set((metricIds ?? []).filter(Boolean)));
  ...
  return fetchSourceCacheValue(
    `bgeometrics:mcp:get-latest:${normalizedMetricIds.join(",")}`,
    async () => {
      const sessionId = await initializeBGeometricsMcpSession();
      return callBGeometricsMcpTool(sessionId, "get_latest", { metrics: normalizedMetricIds });
    },
    options.ttlMs ?? 24 * 60 * 60 * 1000,
  );
}

export async function fetchBGeometricsMetricSeriesMap(metricMap, options = {}) {
  const descriptors = Object.entries(metricMap ?? {})
    .filter(([, metricId]) => Boolean(metricId))
    .map(([key, metricId]) => ({ key, metricId }));
  ...
  for (const descriptor of descriptors) {
    const cacheLabel = `bgeometrics:mcp:get-metric:${descriptor.metricId}`;
    const cacheKey = toCacheKey(cacheLabel);
    const cached = ttlMs > 0 ? await readSourceCache(cacheKey) : null;

    if (cached !== null) {
      results[descriptor.key] = cached;
      continue;
    }

    missing.push({ ...descriptor, cacheKey, cacheLabel });
  }
  ...
  for (const descriptor of missing) {
    try {
      const text = await callBGeometricsMcpTool(sessionId, "get_metric", { metric: descriptor.metricId });
      const csvText = extractMarkdownCsv(text);
      const series = csvText ? parseCsvSeries(csvText) : [];
      results[descriptor.key] = series;

      if (ttlMs > 0) {
        await Promise.all([
          writeSourceCache(descriptor.cacheKey, series, ttlMs),
          writeWatermark(`source:${descriptor.cacheKey}`, {
            cacheKey: descriptor.cacheLabel,
            lastSuccessfulFetchAt: Date.now(),
            ttlMs,
          }),
        ]);
...
export async function fetchBGeometricsCohortMetrics(options = {}) {
  return fetchBGeometricsMetricSeriesMap(
    {
      lthSupplySeries: "long-term-hodler-supply-btc",
      sthSupplySeries: "short-term-hodler-supply-btc",
      lthNetPositionChangeSeries: "lth-net-position-change-30d-btc",
      reserveRiskSeries: "reserve-risk",
      livelinessSeries: "liveliness",
      supplyProfitSeries: "supply-profit",
    },
    {
...
export async function fetchCoinMetricsAssetMetrics(metrics, options = {}) {
  const normalizedMetrics = Array.from(new Set((metrics ?? []).filter(Boolean)));

  if (normalizedMetrics.length === 0) {
    return {};
  }

  const url = buildUrl(COINMETRICS_COMMUNITY_API_BASE, {
    assets: options.asset ?? "btc",
    metrics: normalizedMetrics.join(","),
    frequency: options.frequency ?? "1d",
    page_size: options.pageSize ?? 10000,
  });
  const payload = await fetchCachedValue(url, {
    cacheKey: `coinmetrics:${options.asset ?? "btc"}:${options.frequency ?? "1d"}:${normalizedMetrics.join(",")}`,
...
export async function fetchBGeometricsSeries(path) {
  ...
  return toFiniteSeries(
    (payload ?? []).map((point) => ({
      timestamp: toOptionalNumber(point?.[0]),
      value: toOptionalNumber(point?.[1]),
    })),
  );
}
...
export async function fetchBGeometricsPlotlySeries(path, traceName) {
  ...
  return toFiniteSeries(
    dates.map((date, index) => ({
      timestamp: new Date(date).getTime(),
      value: toOptionalNumber(values[index]),
    })),
  );
}
