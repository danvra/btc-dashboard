...
export async function refreshFastGroup(options = {}) {
  ...
  const warnings = [];
  const localHistory = {};
  const [coingecko, markets, fearGreedSeries, rateProbability] = await Promise.all([
    fetchCoinGeckoPrice(),
...
export async function refreshDailyGroup(options = {}) {
  ...
  const realizedPriceUsd = latestMvrv && currentBtcPrice > 0 ? currentBtcPrice / latestMvrv : null;
  ...
  return createSnapshot("daily", {
    ...
    context: {
      ...
      realizedPriceUsd,
      syntheticInputs: {
        mvrv: mvrvSeries.length > 0 ? mvrvSeries : previousSnapshot?.context?.syntheticInputs?.mvrv,
        "percent-supply-in-profit":
        ...
        "price-vs-realized-price":
          mvrvSeries.length > 0 ? mvrvSeries : previousSnapshot?.context?.syntheticInputs?.["price-vs-realized-price"],
        asopr:
        ...
        liveliness:
          livelinessSeries.length > 0 ? livelinessSeries : previousSnapshot?.context?.syntheticInputs?.liveliness,
        "lth-net-position-change":
