...
function redditRetentionWarning(reason, hasPreviousSnapshot) {
  ...
  return hasPreviousSnapshot
    ? `${reason} Retaining the previous cached Reddit sentiment snapshot.`
    : `${reason} No previous cached Reddit sentiment snapshot was available to retain.`;
}
...
export async function refreshFastGroup(options = {}) {
  ...
  if (redditSentiment && !redditSentiment.ok) {
    const warning = redditRetentionWarning(
      redditSentiment.reason,
  ...
  if (redditSentiment?.ok && redditSentiment.score !== undefined) {
    ...
    metrics["recent-reddit-sentiment"] = buildSnapshotMetric(
      "recent-reddit-sentiment",
      {
        ...
        sourceLabel: `PullPush + ${redditSentiment.model ?? "OpenAI"}`,
