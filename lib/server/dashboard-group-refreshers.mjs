...
function buildFearGreedMetric(series) {
  ...
  const status = latest < 25 ? "bearish" : latest < 60 ? "neutral" : "bullish";
...
export async function refreshDailyGroup(options = {}) {
  ...
  const effectiveMvrvSeries = glassnodeMetrics?.mvrvSeries?.length > 0 ? glassnodeMetrics.mvrvSeries : mvrvSeries;
  ...
  const latestSupply = totalBitcoinsSeries.at(-1)?.value ?? null;
  ...
  if (effectiveMvrvSeries.length > 0) {
    metrics.mvrv = buildMvrvMetric(effectiveMvrvSeries);
  }
  ...
  if (effectiveMvrvSeries.length > 0) {
    metrics["price-vs-realized-price"] = buildMetric(
      ...
      effectiveMvrvSeries,
      `${formatRatio(effectiveMvrvSeries.at(-1)?.value ?? 0, 2)}x`,
      realizedPriceUsd ? `Spot ${formatUsd(latestSpot, 0)} vs realized ${formatUsd(realizedPriceUsd, 0)}` : "Spot / realized price proxy",
      glassnodeMetrics?.realizedPriceSeries?.length > 0 ? "Glassnode" : "Blockchain.com proxy",
      {
        dataMode: glassnodeMetrics?.realizedPriceSeries?.length > 0 ? "live" : "approx",
      },
    );
  }
  ...
  if (effectiveAsoprSeries.length > 0) {
    metrics.asopr = buildMetric(
      ...
      {
        dataMode: glassnodeMetrics?.asoprSeries?.length > 0 ? "live" : asoprIsExact ? "scraped" : "approx",
      },
    );
  }
  ...
  if (effectivePercentSupplyInProfitSeries.length > 0) {
    metrics["percent-supply-in-profit"] = buildMetric(
      ...
      glassnodeMetrics?.percentSupplyInProfitSeries?.length > 0 ? "Glassnode" : "BGeometrics",
      {
  ...
  if (effectiveReserveRiskSeries.length > 0) {
    metrics["reserve-risk"] = buildMetric(
      ...
      glassnodeMetrics?.reserveRiskSeries?.length > 0 ? "Glassnode" : "BGeometrics",
      {
  ...
  if (effectiveLthSupplySeries.length > 0) {
    metrics["lth-supply"] = buildMetric(
      ...
      glassnodeMetrics?.lthSupplySeries?.length > 0 ? "Glassnode" : "BGeometrics",
      {
  ...
  if (effectiveSthSupplySeries.length > 0) {
    metrics["sth-supply"] = buildMetric(
      ...
      glassnodeMetrics?.sthSupplySeries?.length > 0 ? "Glassnode" : "BGeometrics",
      {
  ...
  if (lthNetPositionChangeSeries.length > 0) {
    metrics["lth-net-position-change"] = buildMetric(
      ...
      glassnodeMetrics?.lthNetPositionChangeSeries?.length > 0 ? "Glassnode" : "BGeometrics derived",
      {
  ...
  if (effectiveLivelinessSeries.length > 0) {
    metrics.liveliness = buildMetric(
      ...
      glassnodeMetrics?.livelinessSeries?.length > 0 ? "Glassnode" : "BGeometrics",
      {
