...
export async function refreshFastGroup(options = {}) {
  ...
  const previousSnapshot = options.previousSnapshot ?? null;
  const warnings = [];
  const [coingecko, markets, fundingBinance, fundingBybit, fundingOkx, openInterestPayload, mempoolFees] = await Promise.all([
    fetchCoinGeckoPrice(),
...
export async function refreshSlowGroup(options = {}) {
  ...
  if (!process.env.FRED_API_KEY) {
    warnings.push("FRED_API_KEY is missing, so the slow macro cohort is serving cached values when available.");
  }
