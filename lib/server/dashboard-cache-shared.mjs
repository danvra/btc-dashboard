...
function metricIdsForGroup(groupId) {
...
export function summarizeGroupSnapshot(snapshot, now = Date.now()) {
  ...
  const expiresAt = snapshot.expiresAt ?? (generatedAt && config.ttlMs > 0 ? generatedAt + config.ttlMs : 0);
  ...
  return {
    ...
    generatedAt,
    expiresAt,
    ttlMs: snapshot.ttlMs ?? config.ttlMs,
    ...
    metricIds: snapshot.metricIds ?? metricIdsForGroup(snapshot.groupId),
    status,
  };
}
...
export function buildGroupSnapshotsFromPayload(payload, options = {}) {
  ...
  for (const groupId of CACHE_GROUP_ORDER) {
    ...
    const metricIds = metricIdsForGroup(groupId);
...
export function buildCompositePayloadFromGroupSnapshots(groupSnapshots, basePayload = {}, options = {}) {
  ...
  meta.scheduler =
    ...
    "TTL-managed grouped cache with request-driven refresh";
