...
const serverStateFile = path.join(projectRoot, ".dashboard-server-state.json");
...
async function readLocalServerState() {
  return (await readJsonFile(serverStateFile, {})) ?? {};
}

async function writeLocalServerState(serverState) {
  await writeJsonFile(serverStateFile, serverState ?? {});
}
...
export async function recordPageLoad() {
  if (getStorageMode() === "redis") {
    const nextCount = Number(await redisCommand(["INCR", "dashboard:counter:page-loads"]));
    return Number.isFinite(nextCount) ? Math.max(0, nextCount) : 0;
  }

  const serverState = await readLocalServerState();
  const currentCount = Number(serverState?.pageLoadCount ?? 0);
  const nextCount = Number.isFinite(currentCount) ? currentCount + 1 : 1;

  await writeLocalServerState({
    ...serverState,
    pageLoadCount: nextCount,
  });
