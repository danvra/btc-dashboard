...
async function loadCachedState(persist) {
  ...
  const canPersist = persist && storage.writable;
...
async function refreshSelectedGroups(targetGroupIds, options = {}) {
  ...
  const storage = getStorageCapabilities();
  const persist = options.persist !== false && storage.writable;
  const { baseComposite, groupSnapshots: initialSnapshots, bootstrapUsed } = await loadCachedState(persist);
...
export async function ensureDashboardCache(options = {}) {
  return refreshSelectedGroups(["fast", "daily", "slow", "synthetic"], options);
}

export async function warmDashboardCache(options = {}) {
  return refreshSelectedGroups(["daily", "slow", "synthetic"], {
    ...options,
    force: true,
  });
}
