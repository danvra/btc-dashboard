...
const ANALOG_METRICS = [
  ...
  { id: "asopr", weight: 1.0, family: "damage", direction: "lower" },
  ...
  { id: "lth-net-position-change", weight: 1.0, family: "distribution", direction: "lower" },
];
...
function buildCurrentMetricContexts({ currentMetrics, historicalSeries }) {
  return ANALOG_METRICS.map((metric) => {
    ...
    const quality = QUALITY_MULTIPLIER[metricState.dataMode ?? "approx"] ?? QUALITY_MULTIPLIER.approx;
