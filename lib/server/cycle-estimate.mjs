...
const METRIC_SOURCE_CONFIDENCE = {
  mvrv: 1,
  "puell-multiple": 0.96,
  "pi-cycle-top": 0.9,
  "mayer-multiple": 0.9,
  ...
  "power-law": 0.74,
  "hash-ribbon": 0.9,
  "adjusted-transfer-volume": 0.84,
  "active-addresses": 0.84,
  "active-supply": 0.78,
  "fear-and-greed": 0.74,
  dxy: 0.88,
  "10y-real-yield": 0.88,
...
function getSignalWeight(metricId, metric) {
  return getQualityWeight(metric) * (METRIC_SOURCE_CONFIDENCE[metricId] ?? 0.8);
}
...
function buildSignal(metricId, metric, stageScore, explanation, family) {
  ...
  return {
    ...
    family,
    weight: getSignalWeight(metricId, metric),
...
function stageScoreFromStatus(metric, bearishScore = 0.7, bullishScore = 0.3, neutralScore = 0.5) {
  if (!metric) {
    return null;
  }
...
function adjustStageScoreForStatus(score, status, amount = 0.08) {
  if (!Number.isFinite(score)) {
    return null;
  }

  if (status === "bearish") {
    return clamp(score + amount);
  }
...
function buildDeterministicEstimate(metrics, generatedAt, previousEstimate) {
  ...
  const adjustedTransferVolume = buildSignal(
    ...
    normalizeWithinSeries(metrics["adjusted-transfer-volume"], true) ??
      stageScoreFromStatus(metrics["adjusted-transfer-volume"], 0.72, 0.28, 0.5),
  ...
  const activeAddresses = buildSignal(
    ...
    normalizeWithinSeries(metrics["active-addresses"], true) ??
      stageScoreFromStatus(metrics["active-addresses"], 0.68, 0.32, 0.5),
  ...
  const activeSupply = buildSignal(
    ...
    normalizeWithinSeries(metrics["active-supply"], true) ??
      stageScoreFromStatus(metrics["active-supply"], 0.66, 0.34, 0.5),
  ...
  const openInterest = buildSignal(
    ...
    adjustStageScoreForStatus(
      normalizeWithinSeries(metrics["open-interest"]),
      metrics["open-interest"]?.status,
      0.1,
    ),
